...
class BST:
    ...
    def _remove(self, value : int, node):
        if node.data == value:
            self.delete_branch(node)
        elif node.data > value and node.left != None:
            self._remove(node.left, value)
        elif node.data < value and node.right != None:
    ...
    def delete_branch(self, node):
        if node.left != None:
            self.delete_branch(node.left)
        if node.right != None:
            self.delete_branch(node.right)
        del node
